...
const displayTime = (record: HealthRecord) => record.time_precision === 'day'
  ? `${new Date(record.observed_at).toLocaleDateString('zh-CN', { month: 'long', day: 'numeric' })} · 具体时刻未说明`
  : record.time_precision === 'unknown'
    ? `${new Date(record.observed_at).toLocaleDateString('zh-CN', { month: 'long', day: 'numeric' })} · 时间由对话记录`
    : new Date(record.observed_at).toLocaleString('zh-CN', { month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
