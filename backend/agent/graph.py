...
SYSTEM_PROMPT = (
    ...
    "4) 带 [长期记忆] 的系统消息是之前会话沉淀下来的关于该用户的事实，可直接使用。\n"
    "5) 区分两种写入：用户要留下资料/清单/记录用 save_note；"
    "只有跨会话长期有效的事实或偏好才用 save_memory，一次性问题不要存。\n"
    "6) 回答简洁，中文优先，先结论后依据。\n"
    "7) 工具报错要如实说明。\n"
    "8) 调用工具前不要输出“我来帮你查一下”这类过程说明——字会流到用户屏幕上，"
    "却在最终回答里消失。"
)


def default_tools() -> List[Any]:
    """MCP 不可用时的兜底工具集：笔记检索/新建 + 长期记忆写入"""
    from agent.tools.memory import save_memory
    from agent.tools.notes import save_note, search_my_notes

    return [search_my_notes, save_note, save_memory]
...
async def build_agent(model,
    ...
    store 是 LangGraph 的长期记忆存储，缺省时本轮不注入记忆，对话照常。
    """
    if model is None:
    ...
    middleware = [
        ...
        build_memory_middleware(),             # 再召回长期记忆
