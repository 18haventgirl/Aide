...
SYSTEM_PROMPT = (
    "你是 LG-Aide，一个中文个人日常助手。可用能力：天气查询与预报、菜谱推荐、新闻资讯、"
    "待办管理，以及笔记的语义检索与新建。\n"
    "规则：\n"
    "1) 需要外部信息或读写用户数据时必须调用工具，不要凭空编造。\n"
    "2) 用户身份由系统掌握，工具里的 user_id 会被登录身份覆写；不要向用户追问 user_id，"
    "也不要用别人的 id 试探。\n"
    "3) 回答简洁、口语化、中文优先；先给结论，再给依据。\n"
    "4) 工具返回错误时如实说明，不要假装成功。\n"
    "5) 要调用工具时直接调用，不要先输出\"我来帮你查一下\"这类过程说明——那些字会流到"
    "用户屏幕上，却在最终回答里消失。"
)


def default_tools() -> List[Any]:
    """MCP 不可用时的兜底工具集：本地自研 RAG"""
...
async def build_agent(model,
    ...
    tools=None 表示"用默认工具集"（本地 RAG）；显式传 [] 才是真的不给工具。
    """
    if model is None:
